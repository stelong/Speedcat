...
function piecewise(condition, case1, case2)
	if condition
		case1
	else
		case2
...
function computeAlgebraics(u, p, t)
	...
	i_Stim = piecewise(t-fld(t, stim_period)*stim_period >= 0.0 && t-fld(t, stim_period)*stim_period <= stim_duration, stim_amplitude, 0.0)
	FVRT = F*V/(R*T)
	FVRT_Ca = 2.0*FVRT
	...
	tau_h = piecewise(V >= -40.0, 0.4537*(1.0+exp(-(V+10.66)/11.1)), 3.49/(0.135*exp(-(V+80.0)/6.8)+3.56*exp(0.079*V)+310000.0*exp(0.35*V)))
	j_inf = (1.0+exp((V+76.1)/6.07))^(-1)
	tau_j = piecewise(V >= -40.0, 11.63*(1.0+exp(-0.1*(V+32.0)))*exp(0.0000002535*V), 3.49/((V+37.78)/(1.0+exp(0.311*(V+79.23)))*(-127140.0*exp(0.2444*V)-0.00003474*exp(-0.04391*V))+0.1212*exp(-0.01052*V)/(1.0+exp(-0.1378*(V+40.14)))))
	...
	C_oc = piecewise(abs(FVRT_Ca) > 1e-9, (Ca_i+J_L/g_D*Ca_o*FVRT_Ca*exp(-FVRT_Ca)/(1.0-exp(-FVRT_Ca)))/(1.0+J_L/g_D*FVRT_Ca/(1.0-exp(-FVRT_Ca))), ((Ca_i+J_L*Ca_o/g_D)/(1.0+J_L/g_D)))
	...
	J_Roo = piecewise(abs(FVRT_Ca) > 1e-5, J_R*(Ca_SR-Ca_i+J_L/g_D*FVRT_Ca/(1.0-exp(-FVRT_Ca))*(Ca_SR-Ca_o*exp(-FVRT_Ca)))/(1.0+J_R/g_D+J_L/g_D*FVRT_Ca/(1.0-exp(-FVRT_Ca))), J_R*(Ca_SR-Ca_i+J_L/g_D*1e-5/(1.0-exp(-1e-5))*(Ca_SR-Ca_o*exp(-1e-5)))/(1.0+J_R/g_D+J_L/g_D*1e-5/(1.0-exp(-1e-5))))
	J_Loc = piecewise(abs(FVRT_Ca) > 1e-5, J_L*FVRT_Ca/(1.0-exp(-FVRT_Ca))*(Ca_o*exp(-FVRT_Ca)-Ca_i)/(1.0+J_L/g_D*FVRT_Ca/(1.0-exp(-FVRT_Ca))), J_L*1e-5/(1.0-exp(-1e-5))*(Ca_o*exp(-1e-5)-Ca_i)/(1.0+J_L/g_D*1e-5/(1.0-exp(-1e-5))))
	J_Loo = piecewise(abs(FVRT_Ca) > 1e-5, J_L*FVRT_Ca/(1.0-exp(-FVRT_Ca))*(Ca_o*exp(-FVRT_Ca)-Ca_i+J_R/g_D*(Ca_o*exp(-FVRT_Ca)-Ca_SR))/(1.0+J_R/g_D+J_L/g_D*FVRT_Ca/(1.0-exp(FVRT_Ca))), J_L*1e-5/(1.0-exp(-1e-5))*(Ca_o*exp(-1e-5)-Ca_i+J_R/g_D*(Ca_o*exp(-1e-5)-Ca_SR))/(1.0+J_R/g_D+J_L/g_D*1e-5/(1.0-exp(-1e-5))))
	...
	a = [i_Stim, FVRT, FVRT_Ca, E_Na, i_Na, m_inf, tau_m, h_inf, tau_h, j_inf, tau_j, E_K, i_t, r_inf, tau_r, s_inf, tau_s, s_slow_inf, tau_s_slow, i_ss, r_ss_inf, tau_r_ss, s_ss_inf, tau_s_ss, i_K1, f_K, i_f_Na, i_f_K, i_f, y_inf, tau_y, i_B_Na, i_B_K, nu_1, i_NaK, C_co, C_oc, expVL, alpha_p, alpha_m, beta_poc, beta_pcc, beta_m, epsilon_pco, epsilon_pcc, epsilon_m, mu_poc, mu_pcc, mu_moc, mu_mcc, J_Rco, J_Roo, J_Loc, J_Loo, denom, P1, P2, P3, P4, r_1, r_2, r_3, r_4, r_5, r_6, r_7, r_8, z_4, J_R1, J_R3, i_RyR, J_L1, J_L2, i_LCC1, i_LCC2, i_NCX1, i_NCX2, i_SERCA, i_pCa1, i_pCa2, E_Ca, i_CaB1, i_CaB2, i_SR, i_TRPN, beta_CMDN]
end

function computeRates(du, u, p, t)
	R, T, F, Cm, stim_period, stim_duration, stim_amplitude, Vmyo, Vmyo_uL, VSR_uL, g_Na, Na_o, g_to, a, b, K_o , g_ss , g_K1, g_f, f_Na, g_B_Na, g_B_K, i_NaK_max, gamma1, KdNaio, KdNais, KdNaes, Delta, alpha, Ca_o, g_D, J_R, J_L, N, V_L, del_VL, phi_L, t_L, tau_L, t_R, tau_R, phi_R, theta_R, K_RyR, K_L, a1, b1, c, d, K_mNa, K_mCa, eta, k_sat, g_NCX, g_SERCA, K_SERCA , g_pCa, K_mpCa, g_CaB, g_SRl, k_m_TRPN, k_p_TRPN, B_TRPN, k_CMDN, B_CMDN, EGTA_tot, KmEGTA, tau_s_ss, f_K, alpha_m, beta_m = p
	z_1, z_2, z_3, r, s, s_slow, y, Ca_SR, Ca_i, K_i, Na_i, TRPN, V, h, j, m, r_ss, s_ss = u	
	i_Stim, FVRT, FVRT_Ca, E_Na, i_Na, m_inf, tau_m, h_inf, tau_h, j_inf, tau_j, E_K, i_t, r_inf, tau_r, s_inf, tau_s, s_slow_inf, tau_s_slow, i_ss, r_ss_inf, tau_r_ss, s_ss_inf, tau_s_ss, i_K1, f_K, i_f_Na, i_f_K, i_f, y_inf, tau_y, i_B_Na, i_B_K, nu_1, i_NaK, C_co, C_oc, expVL, alpha_p, alpha_m, beta_poc, beta_pcc, beta_m, epsilon_pco, epsilon_pcc, epsilon_m, mu_poc, mu_pcc, mu_moc, mu_mcc, J_Rco, J_Roo, J_Loc, J_Loo, denom, P1, P2, P3, P4, r_1, r_2, r_3, r_4, r_5, r_6, r_7, r_8, z_4, J_R1, J_R3, i_RyR, J_L1, J_L2, i_LCC1, i_LCC2, i_NCX1, i_NCX2, i_SERCA, i_pCa1, i_pCa2, E_Ca, i_CaB1, i_CaB2, i_SR, i_TRPN, beta_CMDN = computeAlgebraics(u, p, t)
